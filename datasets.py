...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston13':
        ...
        label_queue = {"grass healthy": ['The grass healthy is next to the road', 'The grass healthy is dark green'],
                       ...
                       "non-residential buildings": ['The shapes of the non-residential buildings are inconsistent',
        ...
        img = np.asarray(sio.loadmat(folder + 'Shanghai.mat')['DataCube1'])
        # img = np.asarray(open_file(folder + 'Shanghai.mat')['DataCube1']).transpose(1, 2, 0)

        rgb_bands = [20, 30, 30]

        # gt = np.asarray(open_file(folder + 'Shanghai_7gt.mat')['map'])
        gt = np.asarray(sio.loadmat(folder + 'Shanghai_gt.mat')['gt1'])

        label_values = ["water", "building", "plant"]

        label_queue = {"water": ['The water has a smooth surface', 'The water appears dark blue or black'],
                       "building": ['The building is along the open coast',
                                   'The building is pale green'],
                       "plant": ['Plant grew along the road', 'The plant appear as elongated strip shape'],
                       }
        ignored_labels = [0]

    elif dataset_name == 'Hangzhou':
        # Load the image
        # img = np.asarray(open_file(folder + 'Hangzhou.mat')['ori_data']).transpose(1, 2, 0)

        img = np.asarray(sio.loadmat(folder + 'Hangzhou.mat')['DataCube2'])

        rgb_bands = [20, 30, 30]

        # gt = np.asarray(open_file(folder + 'Hangzhou_7gt.mat')['map'])
        gt = np.asarray(sio.loadmat(folder + 'Hangzhou_gt.mat')['gt2'])
