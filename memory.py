...
class FeaturesMemory(nn.Module):
    def __init__(self, num_classes, feats_channels, transform_channels, out_channels,
                 use_context_within_image=True, num_feats_per_cls=1, use_hard_aggregate=False, memory_data=None,
                 base_momentum=0.9, adjust_by_learning_rate=True, base_lr =0.01):
        ...
        self.adjust_by_learning_rate = adjust_by_learning_rate
        ...
        if memory_data is not None:
            ...
            self.memory = nn.Parameter(torch.zeros(num_classes, num_feats_per_cls, feats_channels, dtype=torch.float),
                                       requires_grad=False)
        ...
        if self.num_feats_per_cls > 1:
            ...
            self.fuse_memory_conv = nn.Sequential(
                nn.Conv2d(feats_channels * self.num_feats_per_cls, feats_channels, kernel_size=1, stride=1, padding=0,
                          bias=False),
                nn.BatchNorm2d(512),
    ...
    def forward(self, feats, preds=None):
        # feats = feats.reshape(feats.shape[0],feats.shape[1],feats.shape[2],feats.shape[3]*feats.shape[4])
    ...
    def update(self, features, segmentation, ignore_index=255, strategy='cosine_similarity', learning_rate=0.01):
        ...
        if self.adjust_by_learning_rate:
            momentum = self.base_momentum / self.base_lr * learning_rate
